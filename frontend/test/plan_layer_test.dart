...
void main() {
  group('Condition evaluator', () {
    test('preserves all AND conditions', () {
      ...
      final group = ConditionGroup(logic: ConditionLogic.and, conditions: [
        const Condition(
            targetRef: 'name',
      ...
      final filtered =
          evaluator.filterRows(rows, group, (targetRef, row) => row[targetRef]);
    ...
    test('supports nested OR conditions', () {
      ...
      final group = ConditionGroup(logic: ConditionLogic.and, conditions: [
        ConditionGroup(logic: ConditionLogic.or, conditions: [
          const Condition(
              targetRef: 'city',
              operator: ConditionOperator.eq,
              value: 'Kolkata',
  ...
  group('Fallback parser', () {
    test('parses categorize and fix all columns', () {
      ...
      final result =
          parser.parse('Categorize and fix all columns.', requestId: 'req-1');
      expect(result.plan, isNotNull);
      expect(result.plan!.operations, hasLength(2));
    });

    test('parses grouped aggregation requests', () {
      final parser = const DeterministicFallbackParser();
      final result =
          parser.parse('Show average rating by city.', requestId: 'req-agg');
      expect(result.plan, isNotNull);
      expect(
          result.plan!.operations.single, isA<GroupedAggregationOperation>());
    });

    test('refuses currency conversion without rate metadata', () {
      final parser = const DeterministicFallbackParser();
      final result = parser.parse('Convert the currency column to INR.',
          requestId: 'req-2');
  ...
  group('Plan model', () {
    test('round-trips advanced analysis operations', () {
      const plan = InsightFlowPlan(
        schemaVersion: '1.0',
        requestId: 'req-4',
        intent: PlanIntent(taskClass: TaskClass.analyze, summary: 'analyze'),
        semanticTargets: [
          SemanticTarget(
              targetId: 'city',
              kind: 'category',
        ...
        operations: [
          ...
          MissingValueAnalysisOperation(
              columns: ['city'], includePercentages: true),
          DuplicateDetectionOperation(
              keys: ['city'], includeCounts: true, caseSensitive: false),
          OutlierDetectionOperation(
              columns: ['rating'], method: OutlierMethod.iqr, threshold: 1.5),
        ],
        output: OutputSpec(
            sheetNameSeed: 'analysis',
      ...
      final roundTripped = InsightFlowPlan.fromJson(json);
      expect(roundTripped.operations, hasLength(5));
      expect(roundTripped.operations[0], isA<GroupedAggregationOperation>());
      expect(roundTripped.operations[1], isA<SummaryStatisticsOperation>());
      expect(roundTripped.operations[2], isA<MissingValueAnalysisOperation>());
      expect(roundTripped.operations[3], isA<DuplicateDetectionOperation>());
      expect(roundTripped.operations[4], isA<OutlierDetectionOperation>());
    });
  });
  ...
  group('Sheet names', () {
    test('creates short understandable names', () {
      ...
      final name = generator.fromRequest(
  ...
  group('Semantic resolution', () {
    test('discovers entity columns locally', () {
      ...
      const snapshot = TableSnapshot(columns: [
        ColumnProfile(
            columnId: 'c1',
  ...
  group('Validator', () {
    test('rejects unresolved required targets', () {
      ...
      final plan = InsightFlowPlan(
          ...
          requestId: 'req-3',
          intent:
              const PlanIntent(taskClass: TaskClass.filter, summary: 'filter'),
          ...
          operations: const [
            FilterRowsOperation(
                where: ConditionGroup(logic: ConditionLogic.and, conditions: [
              Condition(
                  targetRef: 'rating',
                  operator: ConditionOperator.gt,
                  value: 3.5,
      ...
      final result =
          validator.validate(plan, canResolveTarget: (targetId) => false);
      expect(result.ok, isFalse);
      expect(result.unresolvedTargets, contains('rating'));
    });

    test('accepts the advanced analysis operations when targets resolve', () {
      const validator = LocalPlanValidator();
      final plan = InsightFlowPlan(
        schemaVersion: '1.0',
        requestId: 'req-5',
        intent:
            const PlanIntent(taskClass: TaskClass.analyze, summary: 'analysis'),
        semanticTargets: const [
          SemanticTarget(
              targetId: 'city',
              kind: 'category',
      ...
      final result = validator.validate(plan,
          canResolveTarget: (targetId) =>
              targetId == 'city' || targetId == 'rating');
      expect(result.ok, isTrue, reason: result.errors.join(' | '));
      expect(result.unresolvedTargets, isEmpty);
    });
  });
}
