...
async def parse_agentic_command(
    ...
    learned = get_agentic_orchestrator().plan(user_text, available_columns=available_columns)
    if learned.route == "operation" and learned.plan is not None:
        operation = dict(learned.plan)
        operation.setdefault("confidence", learned.confidence)
        operation.setdefault("message", learned.message)
        return operation
