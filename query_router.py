...
def _sql_result_summary(result: dict[str, Any] | pd.DataFrame | None) -> dict[str, Any]:
    if result is None:
        return {}
    if isinstance(result, pd.DataFrame):
        return {
            ...
            "columns": [str(column) for column in result.columns[:10]],
        }
    if isinstance(result, dict):
        columns = result.get("columns")
        if isinstance(columns, list):
            preview_columns = [str(column) for column in columns[:10]]
        else:
            preview_columns = []
...
async def handle_smart_query(
    ...
    learning_orchestrator = get_agentic_orchestrator()
    learning_decision = learning_orchestrator.plan(user_text, df=df, available_columns=available_columns)
    ...
    # sentiment analysis.
    if learning_decision.route == "sql" and learning_decision.plan is not None:
        try:
            sql = build_sql_from_plan(learning_decision.plan, available_columns)
            result = _execute_sql(sql, df)
            learning_orchestrator.record_result(
                user_text=user_text,
                decision=learning_decision,
                df=df,
                available_columns=available_columns,
                result_summary=_sql_result_summary(result),
                success=True,
            )
            return to_json_safe(smart_query_envelope(
                success=True,
                ...
                message=learning_decision.message,
                plan=learning_decision.plan,
                sql=sql,
                result=result,
            ))
        except Exception as e:
            logger.exception("[query_router] Learning fast-path failed")
            learning_orchestrator.record_result(
                user_text=user_text,
                decision=learning_decision,
                df=df,
    ...
    if route == "sql":
        ...
        try:
            ...
            learning_orchestrator.record_result(
                user_text=user_text,
                decision=learning_decision if learning_decision.route == "sql" else learning_orchestrator.plan(user_text, df=df, available_columns=available_columns),
                df=df,
                available_columns=available_columns,
                result_summary={"result_kind": "error"},
                success=False,
    ...
    learning_orchestrator.record_result(
        user_text=user_text,
        decision=learning_decision if learning_decision.route == "operation" else learning_orchestrator.plan(user_text, df=df, available_columns=available_columns),
        df=df,
        available_columns=available_columns,
        result_summary={"result_kind": "operation", "action": op_result.get("action")},
        success=True,
    )
