...
def qwen_semantic_memorization_convergence_cycle(*, stage_root: Path = DEFAULT_STAGE_ROOT, spec: KaggleNotebookSpec | None = None, run_id: str | None = None) -> dict[str, Any]:
    spec = spec or KaggleNotebookSpec(workflow_mode="qwen_semantic_memorization_convergence")
    resolved_run_id = run_id or generate_run_id(git_commit=get_repo_state().head)
    stage_root = _resolve_stage_root(stage_root, resolved_run_id)
    preflight_result = preflight(spec, stage_root=stage_root)
    if not preflight_result["ready"]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("qwen-semantic-memorization-convergence-cycle")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "qwen-semantic-memorization-convergence-cycle":
            _emit_json(qwen_semantic_memorization_convergence_cycle(stage_root=args.stage_root, spec=KaggleNotebookSpec(**{**spec.to_dict(), "workflow_mode": "qwen_semantic_memorization_convergence"}), run_id=args.run_id))
