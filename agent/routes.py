...
@learning_router.get("/plan-templates")
async def list_plan_templates(limit: int = 50):
    orchestrator = get_agentic_orchestrator()
    return {"plan_templates": orchestrator.store.load_plan_templates(limit=limit)}


@learning_router.get("/corrections")
async def list_corrections(limit: int = 50):
    orchestrator = get_agentic_orchestrator()
    return {"corrections": orchestrator.store.load_corrections(limit=limit)}
