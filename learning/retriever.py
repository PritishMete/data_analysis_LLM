...
def score_experience(record: ExperienceRecord, features: QueryFeatures) -> tuple[float, list[str]]:
    ...
    if record.plan_template_id:
        score += 0.06
        reasons.append("template id present")
...
def score_plan_template(template: PlanTemplate, features: QueryFeatures) -> tuple[float, list[str]]:
    reasons: list[str] = []
    score = 0.0
    if template.intent == features.intent:
        score += 0.3
        reasons.append("intent match")
    if template.logical_structure == features.logical_structure:
        score += 0.16
        reasons.append("structure match")
    role_score, matched_roles = _overlap(template.required_roles, features.semantic_roles)
    if matched_roles:
        score += min(0.2, role_score)
        reasons.append(f"roles: {matched_roles}")
    tool_score, matched_tools = _overlap(template.tool_sequence, features.tool_hints)
    if matched_tools:
        score += min(0.14, tool_score)
        reasons.append(f"tools: {matched_tools}")
    score += min(0.08, template.average_quality * 0.08)
    score += min(0.08, template.support_count * 0.02)
    if template.state in {"validated", "trusted"}:
        score += 0.08
    if template.state == "trusted":
        score += 0.05
    if score <= 0.05:
        return 0.0, []
    return round(min(score, 0.99), 4), reasons
...
def score_correction_record(record: CorrectionRecord, features: QueryFeatures) -> tuple[float, list[str]]:
    reasons: list[str] = []
    score = 0.0
    if record.affected_intent == features.intent:
        score += 0.28
        reasons.append("intent match")
    if record.dataset_semantic_signature and record.dataset_semantic_signature == features.dataset_semantic_signature:
        score += 0.18
        reasons.append("dataset match")
    if record.requested_role and record.requested_role in features.semantic_roles:
        score += 0.2
        reasons.append("requested role match")
    if record.preferred_semantic_candidate and record.preferred_semantic_candidate in features.tool_hints:
        score += 0.08
    if score <= 0.04:
        return 0.0, []
    return round(min(score, 0.99), 4), reasons
...
def rank_templates(templates: list[PlanTemplate], features: QueryFeatures, limit: int = 5) -> list[dict[str, Any]]:
    return rank_records(templates, score_plan_template, features, limit=limit)
