...
class DataCleaner:
    ...
    def normalize_text_columns(self) -> "DataCleaner":
        ...
        for col in text_cols:
            ...
            # Convert to string, strip whitespace, and keep the replacement
            # logic on the Python string engine so Arrow-backed string arrays
            # do not try to compile Unicode escapes the regex engine rejects.
            cleaned = self.df[col].astype(str).str.strip()
            cleaned = cleaned.map(lambda value: re.sub(r'\s+', ' ', value) if isinstance(value, str) else value)
            cleaned = cleaned.map(lambda value: re.sub(r'[\u200b\u200c\u200d\ufeff\u00ad]', '', value) if isinstance(value, str) else value)
            self.df[col] = cleaned
